...
def aggregate_sales(spark, jdbc_url, db_properties):
    ...
    fact_sales = spark.read.jdbc(
        url=jdbc_url, table='processed.fact_sales', properties=db_properties
    )

    # Read dimensions
    dim_date = spark.read.jdbc(
        url=jdbc_url, table='processed.dim_date', properties=db_properties
    )

    dim_products = spark.read.jdbc(
        url=jdbc_url, table='processed.dim_products', properties=db_properties
    )

    dim_customers = spark.read.jdbc(
        url=jdbc_url, table='processed.dim_customers', properties=db_properties
    )

    # Join datasets
    sales_full = (
        fact_sales.join(dim_date, fact_sales.date_key == dim_date.date_key)
    ...
    monthly_category = (
        sales_full.groupBy(
            col('year'), col('month'), col('month_name'), col('category')
        )
        .agg(
    ...
    customer_spending = (
        sales_full.groupBy(
            col('customer_id'), col('customer_name'), col('city'), col('state')
        )
        .agg(
    ...
    payment_analysis = (
        sales_full.groupBy('payment_method')
        .agg(
            count('*').alias('transaction_count'),
            spark_round(sum('total_amount'), 2).alias('total_revenue'),
            spark_round(avg('total_amount'), 2).alias('avg_transaction_value'),
