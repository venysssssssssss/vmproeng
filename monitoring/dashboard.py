...
def main():
    ...
    # Tabs principais
    tab1, tab2, tab3, tab4 = st.tabs(["📊 Visão Geral", "🔄 DAGs", "📈 Dados", "🎲 Gerador Faker"])
...
def show_overview():
    ...
    try:
        ...
        # Revenue total
        data, _ = execute_query("SELECT COALESCE(SUM(total_amount), 0) FROM raw.sales_transactions WHERE status = 'Completed'")
        if data:
            col2.metric("Revenue Total", f"R$ {data[0][0]:,.2f}")
        
        # Clientes únicos
        data, _ = execute_query("SELECT COUNT(DISTINCT customer_id) FROM raw.sales_transactions")
        if data:
            col3.metric("Clientes Únicos", data[0][0])
        
        # Produtos únicos
        data, _ = execute_query("SELECT COUNT(DISTINCT product_id) FROM raw.sales_transactions")
        if data:
        ...
        if data:
            df = pd.DataFrame(data, columns=['Categoria', 'Transações', 'Revenue'])
            
            col1, col2 = st.columns(2)
            
            with col1:
                fig = px.bar(df, x='Categoria', y='Transações', 
                           title='Transações por Categoria')
                st.plotly_chart(fig, use_container_width=True)
            
            with col2:
                fig = px.pie(df, values='Revenue', names='Categoria',
...
def show_dags():
    ...
    dags = [
        {"id": "ecommerce_etl_pipeline", "name": "Pipeline ETL E-commerce", "status": "active"},
        {"id": "data_quality_check", "name": "Verificação de Qualidade", "status": "active"},
        {"id": "api_data_ingestion", "name": "Ingestão de APIs", "status": "paused"},
        {"id": "minio_data_lake", "name": "Pipeline Data Lake", "status": "active"}
    ]
    ...
    with col2:
        ...
        st.metric("DAGs Ativas", len([d for d in dags if d['status'] == 'active']))
        st.metric("DAGs Pausadas", len([d for d in dags if d['status'] == 'paused']))
        
        st.info("💡 **Dica**: Use o Airflow UI em http://localhost:8080 para controle avançado das DAGs")
...
def show_data_layers():
    ...
    # Seleção de camada
    layer = st.selectbox("Escolha a camada:", 
    ...
    try:
        if layer == "Raw":
            ...
            data, columns = execute_query("SELECT * FROM raw.sales_transactions ORDER BY created_at DESC LIMIT 100")
            ...
            if data and columns:
                ...
                col2.metric("Última Atualização", df['created_at'].max() if not df.empty else "N/A")
                col3.metric("Revenue Total", f"R$ {df['total_amount'].sum():,.2f}" if not df.empty else "R$ 0,00")
            else:
                st.info("Nenhum dado encontrado na camada Raw")
        
        elif layer == "Staging":
            st.subheader("🔄 Camada Staging")
            data, columns = execute_query("SELECT * FROM staging.sales_clean ORDER BY processed_at DESC LIMIT 100")
            ...
            data, columns = execute_query("SELECT * FROM processed.dim_customers LIMIT 50")
            if data and columns:
                df = pd.DataFrame(data, columns=columns)
                st.dataframe(df, use_container_width=True)
            
            st.write("**Dimensão Produtos**")
            data, columns = execute_query("SELECT * FROM processed.dim_products LIMIT 50")
            if data and columns:
                df = pd.DataFrame(data, columns=columns)
                st.dataframe(df, use_container_width=True)
            
            st.write("**Fato Vendas**")
            data, columns = execute_query("SELECT * FROM processed.fact_sales LIMIT 50")
            if data and columns:
                df = pd.DataFrame(data, columns=columns)
                st.dataframe(df, use_container_width=True)
            ...
            # Resumo diário
            data, columns = execute_query("SELECT * FROM analytics.daily_sales_summary ORDER BY summary_date DESC")
            if data and columns:
                df = pd.DataFrame(data, columns=columns)
                st.dataframe(df, use_container_width=True)
                
                if not df.empty:
                    fig = px.line(df, x='summary_date', y='total_revenue',
...
def show_faker_generator():
    ...
    st.info("💡 Esta interface permite gerar dados sintéticos para testar o pipeline")
    ...
    with col2:
        ...
        if st.button("🚀 Gerar Dados", type="primary", use_container_width=True):
            ...
            if not categories or not payment_methods:
                st.error("Selecione pelo menos uma categoria e um método de pagamento")
                return
            ...
            try:
                ...
                for i in range(num_records):
                    ...
                    product_names = {
                        ...
                        "Home & Garden": ["Chair", "Table", "Plant", "Lamp", "Vase"],
                        "Sports": ["Ball", "Racket", "Bike", "Weights", "Shoes"],
                    ...
                    """, (transaction_id, random_date, customer_id, product_id, product_name,
                          category, quantity, unit_price, total_amount, payment_method, 'Completed'))
                ...
                st.success(f"🎉 {num_records} registros inseridos na camada Raw!")
