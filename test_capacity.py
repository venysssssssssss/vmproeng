...
def run_capacity_test(batch_size=1000, max_batches=100, delay=2):
    ...
    for batch_num in range(1, max_batches + 1):
        ...
        # Gerar dados
        print(
            f'Batch {batch_num}/{max_batches}: Gerando {batch_size:,} registros...',
            end=' ',
            flush=True,
        )
        transactions, customers, products = generate_batch(
        ...
        if batch_num % 10 == 0:
            ...
            print(
                f'   • Tempo médio/batch: {sum(batch_times)/len(batch_times):.2f}s'
            )
